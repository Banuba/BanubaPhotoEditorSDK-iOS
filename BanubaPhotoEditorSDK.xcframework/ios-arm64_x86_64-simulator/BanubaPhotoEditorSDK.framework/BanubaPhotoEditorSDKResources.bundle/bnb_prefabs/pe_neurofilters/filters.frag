...
void main()
{
    ...
    float sharpen_amount = 
        filter_noise_strength_monochromatic_sharpen_amount_radius.z;
    if (sharpen_amount != 0.) {
        float r = max(1., 
            filter_noise_strength_monochromatic_sharpen_amount_radius.w 
                * (1280. * bnb_SCREEN.w));
        float sigma = (r * 2. - 1.) / 4.;
        ...
        for (int y = 0; y <= ir; ++y) {
            for (int x = 1; x <= ir; ++x) {
                ...
                mult_sum += mult * 4.;
                blurred += mult * (
                    texelFetch(BNB_SAMPLER_2D(tex_input), 
                        clamp(iuv + ivec2(x, y), ivec2(0), isz1), 0).rgb 
                    + texelFetch(BNB_SAMPLER_2D(tex_input), 
                        clamp(iuv + ivec2(-y, x), ivec2(0), isz1), 0).rgb 
                    + texelFetch(BNB_SAMPLER_2D(tex_input), 
                        clamp(iuv + ivec2(-x, -y), ivec2(0), isz1), 0).rgb 
                    + texelFetch(BNB_SAMPLER_2D(tex_input), 
                        clamp(iuv + ivec2(y, -x), ivec2(0), isz1), 0).rgb);
    ...
    float noise_strength =
        filter_noise_strength_monochromatic_sharpen_amount_radius.x;
    if (noise_strength != 0.) {
        float monochromatic =
            filter_noise_strength_monochromatic_sharpen_amount_radius.y;
        vec3 noise = hash(var_uv * bnb_SCREEN.xy) * 2. - 1.;
        noise *= filter_noise_strength_monochromatic_sharpen_amount_radius.x;
    ...
    img.rgb = mix(
        vec3(dot(img.rgb, vec3(0.299, 0.587, 0.114))),
        img.rgb,
        saturation
    );
