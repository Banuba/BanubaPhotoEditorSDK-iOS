...
BNB_DECLARE_SAMPLER_2D(0, 1, tex_pe_makeup);
BNB_DECLARE_SAMPLER_2D(2, 3, tex_pe_blushes);
...
void main()
{
    ...
    bnb_FragColor = blend(
        bnb_FragColor,
        vec4(0., 0., 0., pe_blushes_strength.w),
        BNB_PASS_SAMPLER_ARGUMENT(tex_pe_blushes)
    );

    bnb_FragColor = blend(
        bnb_FragColor,
        vec4(0., 0., 0., pe_makeup_strength.w),
        BNB_PASS_SAMPLER_ARGUMENT(tex_pe_makeup)
    );
}
